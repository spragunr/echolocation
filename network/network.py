...
def main():
    ...
    if not args.test:
        ...
        model = build_and_train_model(x_train, y_train, args.model_folder,
                                      args.lr, args.lr_reduce_every, args.epochs)
...
def raw_generator(x_train, y_train, batch_size=64, shift=.01,
                  noise=.05, no_shift=False, shuffle=True, tone_noise=.25):
...
def build_and_train_model(x_train, y_train, model_folder, lr,
    ...
    hist = net.fit_generator(train_gen,
                             ...
                             validation_steps=end_val//batch_size)

    net.save(model_folder+'/model.h5')
    return net
