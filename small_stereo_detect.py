...
#	print "concatenated audio and depth data saved as 'all_orig.npz'\n"
	...
	np.savez('small_all', audio=audio, depth=new_depth)
	print "concatenated audio and downsized depth data saved as 'small_all.npz'\n"
...
def preprocess_data():
	...
	path = os.getcwd()
	data = np.load(path+'/small_all.npz')
	audio = data['audio']
	depth = data['depth'] # shape: 1050, 12, 16

	depth_reshaped = np.reshape(depth, (depth.shape[0],-1)) # shape: 1050, 192
	...
	print "VALID DEPTH SHAPE",valid_depth.shape
	
	new_audio = audio[valid_bool]

	if os.path.isfile('small_input_spectrograms.npy'):
		print "fetching spectrogram array..."
		input_set = np.load(path+'/small_input_spectrograms.npy')
	else: 
		print "creating spectrogram set 0" 
		freq1, time1, spectro1 = signal.spectrogram(new_audio[0,:,0], noverlap=100)
		freq2, time2, spectro2 = signal.spectrogram(new_audio[0,:,1], noverlap=100)
		dims = spectro1.shape
		...
		for i in range(1,new_audio.shape[0]):
			...
			freq1, time1, spectro1 = signal.spectrogram(new_audio[i,:,0], noverlap=100)
			freq2, time2, spectro2 = signal.spectrogram(new_audio[i,:,1], noverlap=100)
			input_set[i,:,:,0] = spectro1
			input_set[i,:,:,1] = spectro2
#			plt.pcolormesh(spectro2)
#		plt.show()
		np.save('small_input_spectrograms.npy', input_set)
		print "array of spectrograms saved as 'small_input_spectrograms.npy'"
...
def split_data(x, y):
	...
	xtest = x[xbool].reshape((154,129,15,2))
	ytest = y[ybool].reshape((154,192))
	print "reshaping training vectors..."
	xtrain = x[np.logical_not(xbool)].reshape((650,129,15,2))
	ytrain = y[np.logical_not(ybool)].reshape((650,192))
...
def build_model(x_train, y_train):
	...
	net.save('small_stereo_model.h5')
	print "model saved as 'small_stereo_model.h5'"
	return load_model('small_stereo_model.h5')
...
def  main():
	while not os.path.isfile('small_all.npz'):
		get_data()
	input_set, target = preprocess_data()
	x_train, y_train, x_test, y_test = split_data(input_set, target)
	if not os.path.isfile('small_stereo_model.h5'):
		print "building model..."
		model = build_model(x_train, y_train)
	else: 
		model = load_model('small_stereo_model.h5')
