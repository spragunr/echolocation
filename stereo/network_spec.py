...
def build_and_train_model(x_train, y_train):
  ...
  net.add(Conv2D(32, (5,5), strides=(1,1), activation='relu'))
  net.add(Flatten())
  net.add(Dense(200, activation='relu'))
  net.add(Dense(192, activation='linear'))
  net.compile(optimizer='adam', loss=adjusted_mse)
  print "finished compiling"
  net.fit(x_train, y_train, validation_split=0.2, epochs=200, batch_size=32)
...
def main():
  ...
  if argv[1] == "1":
    ...
    with h5py.File(path+'/model_sets_spec4.h5', 'r') as sets:
      x_train = np.log(1+sets['xtrain'][:])
      y_train = np.log(1+sets['ytrain'][:])
      x_test = np.log(1+sets['xtest'][:])
